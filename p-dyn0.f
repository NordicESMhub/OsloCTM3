c-----------------------------------------------------------------------
c---(pdyn0.f)      p-code 6.1a  Alternate 2 v4 (Prather 6/2011)
...
C-----------------------------------------------------------------------
     ...
     &       ,DALFAI(IPAR)
      ...
      integer :: IIX,JJX,IMH,I,J,L,II,III
      logical :: LSP,LNP,LEW
      integer :: IM, JM, LM
...
c<<<<diagnostic only:
        RMSERR1 = RMSERR1/real(IM*JM, r8)
        RMSERR2 = RMSERR2/real(IM*JM, r8)
...
c---Operator order #2:  DYN2U does transport for I=1:IM, J=2:JM-1, L=1:LM
c---    must combine J=1:2 and J=JM-1:JM
      do L = 1,LM
         do I = 1,IM
          AIR2(I,2,L) = AIR2(I,1,L) + AIR2(I,2,L)
          AIR2(I,JM-1,L) = AIR2(I,JM-1,L) + AIR2(I,JM,L)
          AIRMN(I,2,L) = AIRMN(I,1,L) + AIRMN(I,2,L)
          AIRMN(I,JM-1,L) = AIRMN(I,JM-1,L) + AIRMN(I,JM,L)
         enddo
        ...
        enddo
        do I = 1,IM
          AIR3(I,2,L) = AIR2(I,2,L) + DTFIX*(
     &      ALFA(I,2,L)-ALFA(I+1,2,L)+ALFA(I,1,L)-ALFA(I+1,1,L))
          AIR3(I,JM-1,L) = AIR2(I,JM-1,L) + DTFIX*(
     &      ALFA(I,JM,L)-ALFA(I+1,JM,L)+ALFA(I,JM-1,L)-ALFA(I+1,JM-1,L))
        enddo

      enddo

c---Operator order #3:  DYN2V does transport for I=1:IM, J=2:JM-1, L=1:LM
      do L = 1,LM
       do J = 3,JM-2
        do I = 1,IM
       ...
       enddo
        do I = 1,IM
          AIR4(I,2,L) = AIR3(I,2,L) + DTFIX*(-BETA(I,3,L))
          AIR4(I,JM-1,L) = AIR3(I,JM-1,L) + DTFIX*BETA(I,JM-1,L)
        enddo
      enddo
      ...
      do L = 1,LM
       do J = 2,JM-1
        do I = 1,IM
          AIRMIN = min(AIR2(I,J,L),AIR3(I,J,L),AIR4(I,J,L))
          ARATIO = min(ARATIO,AIRMIN/AIRMN(I,J,L))
        enddo
       enddo
      enddo

c-------- adjust time step ---------------------------------------------
      ARATIO = min(0.99_r8,ARATIO)
      DTMAX  = DTFIX * CFLLIM / (1._r8 - ARATIO)
      NADV   = int(DTOPS/DTMAX) + 1

!      !// Some meteorological data may have very large convective mass
!      !// flux (CWETE), such as the CESM1.2, giving negative mass after
!      !// vertical advection - i.e. the calculation of NADV is not
!      !// suficciently well formulated.
!      !// As a crude fix, it is possible to check the CWETE flux against
!      !// AIR, but since multiplying by DTOPS was not enough, I have
!      !// also multiplied by 2.
!      CNVXX = 0
!      do L = 1,LM
!        do J = 2,JM-1
!           do I = 1,IM
!              CNST = CWETE(I,J,L)*DTOPS*2._r8
!              if (CNST .ge. AIR(I,J,L)) then
!                 CNVXX = max(CNVXX,int(CNST/AIR(I,J,L)))
!              end if
!           end do
!        end do
!      end do
