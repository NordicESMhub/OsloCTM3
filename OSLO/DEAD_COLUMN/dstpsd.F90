...
contains
  ...
  subroutine dst_psd_src_ini()
    ...
    real(r8) mss_frc_srcx(dst_src_nbr) ! [frc] Mass fraction of source mode
  ...
  ! She84 p. 75 Table 1 with gsd = 2.0 rather than 3.2 modification from SBG98 p. 10581 Table 1
    mss_frc_srcx(:)=(/2.6e-6_r8,0.781_r8,0.219_r8/) ! [frc] Mass fraction She84 p. 75 Table 1
    dmt_vma_srcx(:) = (/ 0.0111d-6,  2.524d-6, 42.10d-6 /)  ! [m] Mass median diameter She84 p. 75 Table 1
    gsd_anl_srcx(:) = (/ 1.89d0   ,  2.0d0   ,  2.13d0  /)  ! [frc] Geometric standard deviation She84 p. 75 Table 1
#endif
...
! 20020316: Switch from She84 to Dal87 as default
  ...
  ! These modes also summarized in BSM96 p. 73 Table 2 
    mss_frc_srcx(:)=(/0.036_r8,0.957_r8,0.007_r8/) ! [frc] Mass fraction BSM96 p. 73 Table 2
    dmt_vma_srcx(:)=(/ 0.832d-6 ,  4.82d-6 , 19.38d-6 /) ! [m] Mass median diameter BSM96 p. 73 Table 2
    gsd_anl_srcx(:)=(/ 2.10d0   ,  1.90d0  ,  1.60d0  /) ! [frc] Geometric standard deviation BSM96 p. 73 Table 2
#endif /* Dal87 */

#ifdef AlG01
  ! Alfaro and Gomes production modes AlG01 p. 18076 Table 1
  ! Mass fraction taken from BSM96 for lack of better alternative
    mss_frc_srcx(:)=(/0.036_r8,0.957_r8,0.007_r8/) ! [frc] Mass fraction BSM96 p. 73 Table 2 (ad hoc)
    dmt_vma_srcx(:) = (/ 1.5d-6   ,  6.7d-6  , 14.2d-6  /) ! [m] Mass median diameter AlG01 p. 18076 Table 1
    gsd_anl_srcx(:) = (/ 1.7d0    ,  1.6d0   ,  1.5d0   /) ! [frc] Geometric standard deviation AlG01 p. 18076 Table 1
#endif /* AlG01 */
  ...
  ! PaG77 nomenclature:  / Mode C   ,  Mode A  ,  Mode B  /
    mss_frc_srcx(:)=(/0.036_r8,0.957_r8,0.007_r8/) ! [frc] Mass fraction BSM96 p. 73 Table 2 (ad hoc)
    dmt_vma_srcx(:) = (/ 0.27d-6  ,  5.6d-6  ,  57.6d-6 /) ! [m] Mass median diameters PaG77 p. 2080 Table 1 
    gsd_anl_srcx(:) = (/ 1.88d0   ,  2.2d0   ,  1.62d0  /) ! [frc] Geometric standard deviation PaG77 p. 2080 Table 1
  ...
  subroutine dst_psd_ini() ! [fnc] Initialize particle size distributions
    ...
    use cmn_oslo, only: dustbinsradii
    implicit none
    ! Parameters
    ...
    real(r8),parameter::dmt_min_min=0.06d-6; ! [m] Minimum grid diameter
    real(r8),parameter::dmt_max_max=50.0d-6; ! [m] Maximum grid diameter
    ...
    real(r8),dimension(5),parameter::dmt_grd_04=(/ 0.1d-6 ,  1.0d-6,  2.5d-6,  5.0d-6, 10.0d-6 /) ! [m] Particle diameter grid
    ...
    real(r8),parameter::gsd_anl_dfl=2.d0 ! [frc] Geometric standard deviation
    ...
    real(r8),parameter::dns_aer_dfl=2.5d+3 ! [kg m-3] Aerosol density
    ...
    do idx=1,dst_nbr            
       dmt_vma(idx)=2.524d-6  ! [m] Mass median diameter analytic She84 p. 75 Table 1
    ...
    mss_frc_trn_dst_src(:)= & ! [frc] Fraction of transported dust mass at source
         mss_frc_trn_dst_src(:)/ovr_src_snk_mss_ttl
    ...
    write (6,'(a,f7.3)') 'Total transported mass fraction of dust flux = ',ovr_src_snk_mss_ttl
    ...
    do idx=1,dst_nbr
       write (6,'(i3,5(es10.3,1x))') idx, &
            ...
            gsd_anl(idx),dns_aer(idx),mss_frc_trn_dst_src(idx)
    end do                     ! end loop over cst
    ...
    write (6,'(a)') '------------------------------------------------------------'
    !// Make mass weighted raidus resolved available in cmn_oslo.f
    dustbinsradii(:) = dmt_vwr(:) * 0.5d0
    write(*,'(a)') '* Dust radii stored in cmn_oslo: dstbinradii'
    write (6,'(a)') '------------------------------------------------------------'
  ...
  subroutine grd_mk(sz_grd_typ, & ! I
    ...
    if (sz_grd_typ == grd_typ_lnr) then
       ...
       series_ratio = max_min_ratio**(1.0_r8/sz_nbr)
