...
contains
  ...
  subroutine wnd_frc_thr_slt_get( &
    ...
    real(r8),intent(in)::dns_mdp       ! [kg m-3] Midlayer density
    ! Output
    real(r8),intent(out)::wnd_frc_thr_slt ! [m s-1] Threshold friction velocity for saltation
    ! Local
    ...
    wnd_frc_thr_slt = tmp1/sqrt(dns_mdp) ! [m s-1] Threshold friction velocity for saltation dry ground 
  ...
  subroutine wnd_rfr_thr_slt_get( &
    ...
    ! Parameters
    ! Input
    real(r8),intent(in)::wnd_frc       ! [m s-1] Friction velocity
    real(r8),intent(in)::wnd_frc_thr_slt ! [m s-1] Threshold friction velocity for saltation
    real(r8),intent(in)::wnd_mdp       ! [m s-1] Surface layer mean wind speed
    real(r8),intent(in)::wnd_rfr       ! [m s-1] Wind speed at reference height
    ! Output
    real(r8),intent(out)::wnd_rfr_thr_slt ! [m s-1] Threshold 10 m wind speed for saltation
    ! Local
    ! Main Code
    ...
    wnd_rfr_thr_slt = wnd_frc_thr_slt*wnd_rfr/wnd_frc ! [m s-1]
  ...
  subroutine vwc2gwc( &
    ...
    ! Input
    logical,intent(in)::flg_mbl ! [flg] Mobilization candidate flag
    real(r8),intent(in)::vwc_sat ! [m3 m-3] Saturated volumetric water content (sand-dependent)
    real(r8),intent(in)::vwc_sfc ! [m3 m-3] Volumetric water content
    ! Output
    real(r8),intent(out)::gwc_sfc ! [kg kg-1] Gravimetric water content
    ! Local
    real(r8) dns_blk_dry ! [kg m-3] Bulk density of dry surface soil
    ! Main Code
    ! Initialize output
    if (flg_mbl) then
       ...
       dns_blk_dry = dns_prt_sfc*(1.0-vwc_sat) ! [kg m-3] Bulk density of dry surface soil
       gwc_sfc = vwc_sfc*dns_H2O_lqd_std/dns_blk_dry ! O [kg kg-1] Gravimetric water content
    endif ! endif flg_mbl
  ...
  subroutine frc_thr_ncr_wtr_get( &
    ...
    real(r8),intent(in)::mss_frc_cly ! [frc] Mass fraction of clay
    real(r8),intent(in)::gwc_sfc ! [kg kg-1] Gravimetric water content
    ! Output
    real(r8),intent(out)::frc_thr_ncr_wtr ! [frc] Factor by which moisture increases threshold friction velocity
    ! Local
    real(r8) gwc_thr       ! [kg kg-1] Threshold gravimetric water content
    ! Main Code
    ! Initialize output
    ...
    if (flg_mbl) then
       ...
       gwc_thr = 0.17_r8+0.14_r8*mss_frc_cly ! [m3 m-3] 
       if (gwc_sfc > gwc_thr) &
            frc_thr_ncr_wtr = sqrt(1.0_r8+1.21_r8*(100.0_r8*(gwc_sfc-gwc_thr))**0.68_r8) ! [frc] FMB99 p. 155 (15)
    endif ! endif flg_mbl
  ...
  subroutine frc_thr_ncr_drg_get( &
    ...
    ! Output
    real(r8),intent(out)::frc_thr_ncr_drg ! [frc] Factor by which roughness increases threshold friction velocity
    ! Local
  ...
  subroutine wnd_frc_slt_get( &
    ...
    ! Input
    logical,intent(in)::flg_mbl    ! I [flg] Mobilization candidate flag
    real(r8),intent(in)::wnd_frc       ! I [m s-1] Surface friction velocity
    real(r8),intent(in)::wnd_rfr       ! I [m s-1] Wind speed at reference height
    real(r8),intent(in)::wnd_rfr_thr_slt ! I [m s-1] Threshold 10 m wind speed for saltation
    ! Output
    real(r8),intent(out)::wnd_frc_slt   ! O [m s-1] Saltating friction velocity
    ! Local
    ...
    if (flg_mbl.and.wnd_rfr >= wnd_rfr_thr_slt) then
  ...
  subroutine flx_mss_CaCO3_msk( &
    ...
    real(r8),intent(in)::mss_frc_CaCO3 ! I [frc] Mass fraction of CaCO3
    real(r8),intent(in)::mss_frc_cly ! I [frc] Mass fraction of clay
    real(r8),intent(in)::mss_frc_snd ! I [frc] Mass fraction of sand
    ! Output
    ! Input/Output
    ! Fluxes refer to dust on input and to CaCO3 on output
    real(r8),intent(inout)::flx_mss_vrt_dst_CaCO3(dst_nbr) ! I/O [kg m-2 s-1] Vertical mass flux of dust on input, CaCO3 on output
    integer rcd               ! [rcd] Return success code
    ! Local
    integer m                 ! [idx] Counting index
    real(r8) mss_frc_slt   ! [frc] Mass fraction of silt
    ...
    do m=1,dst_nbr
       ...
       if (flg_mbl) then
          ...
          mss_frc_slt = & ! [frc] Mass fraction of silt
               max(0.0_r8,1.0_r8-mss_frc_cly-mss_frc_snd)
          ...
          mss_frc_CaCO3_cly = max(0.0_r8,-0.045_r8+0.5_r8*min(0.5_r8,mss_frc_cly)) ! [frc] Fraction of CaCO3 in clay CSB99 p. 22249 Figure 1b
          mss_frc_CaCO3_slt = max(0.0_r8,-0.175_r8+1.4_r8*min(0.5_r8,mss_frc_slt)) ! [frc] Fraction of CaCO3 in silt CSB99 p. 22249 Figure 1a
          ...
          if (mss_frc_CaCO3 < 0.0_r8 .or. mss_frc_CaCO3 > 1.0_r8) stop &
               'dst: flx_mss_CaCO3_msk() mss_frc_CaCO3 < 0.0.or.mss_frc_CaCO3 > 1.0'
#endif /* not DST_DBG */
             
          ! Convert dust flux to CaCO3 flux
          flx_mss_vrt_dst_CaCO3(m) = flx_mss_vrt_dst_CaCO3(m) & ! [kg m-2 s-1]
               *mss_frc_CaCO3 & ! [frc] Mass fraction of CaCO3 (at this location)
  ...
  subroutine flx_mss_hrz_slt_ttl_Whi79_get( &
    ...
    real(r8),intent(in)::wnd_frc       ! I [m s-1] Surface friction velocity
    real(r8),intent(in)::wnd_frc_thr_slt ! I [m s-1] Threshold friction speed for saltation
    ! Output
    real(r8),intent(out)::flx_mss_hrz_slt_ttl ! O [kg m-1 s-1] Vertically integrated streamwise mass flux
    integer rcd               ! [rcd] Return success code
    ! Local
    ...
    if (flg_mbl .and. wnd_frc > wnd_frc_thr_slt) then
       wnd_frc_rat = wnd_frc_thr_slt/wnd_frc ! [frc]
       flx_mss_hrz_slt_ttl = & ! [kg m-1 s-1] 
            cst_slt*dns_mdp*(wnd_frc**3.0_r8)* &
  ...
  subroutine flx_mss_vrt_dst_ttl_MaB95_get( &
    ...
    !//
    !// Rewritten for Oslo CTM3, works on only one (lon_idx,lat_idx).
    !// Removed longitude loop. Added else part.
    !//
    !// Amund Sovde, October 2009
    !// ------------------------------------------------------------------
    !use pmgrid ! [mdl] Spatial resolution parameters
    ...
    ! Input
    logical,intent(in)::flg_mbl    ! I [flg] Mobilization candidate flag
    real(r8),intent(in)::flx_mss_hrz_slt_ttl ! I [kg m-1 s-1] Vertically integrated streamwise mass flux
    real(r8),intent(in)::mss_frc_cly   ! I [frc] Mass fraction clay 
    ! Output
    real(r8),intent(out)::dst_slt_flx_rat_ttl ! O [m-1] Ratio of vertical dust flux to streamwise mass flux
    real(r8),intent(out)::flx_mss_vrt_dst_ttl ! O [kg m-2 s-1] Total vertical mass flux of dust
    integer rcd               ! [rcd] Return success code
    ! Local
    ...
    if (flg_mbl) then
       ...
       mss_frc_cly_vld = min(mss_frc_cly,0.2_r8) ! [frc]
       dst_slt_flx_rat_ttl = & ! [m-1]
            100.0_r8*exp(ln10*(13.4_r8*mss_frc_cly_vld-6.0_r8)) ! MaB95 p. 16423 (47)
       flx_mss_vrt_dst_ttl = flx_mss_hrz_slt_ttl*dst_slt_flx_rat_ttl ! [kg m-1 s-1] 
    else
  ...
  subroutine mss_frc_src_AlG01_get(&
    ...
    !// Amund Sovde, October 2009
    !// ------------------------------------------------------------------
    use dstgrd, only: dst_src_nbr, wnd_frc_nbr, bln_nbr
    !use pmgrid,only:plon,plond
    implicit none
    
    ! OUTPUT
    real(r8), intent(out):: mss_frc_src(dst_src_nbr)  ! O [frc] mass fraction of source modes
    ! INPUT
    real(r8), intent(in) :: mss_frc_src_lut(wnd_frc_nbr,bln_nbr,dst_src_nbr) ! I Look up table for mss_frc_src
    real(r8), intent(in) :: wnd_frc            ! I [m/s] wind friction speed
    real(r8), intent(in) :: sfc_frc_bln(bln_nbr) ! [frc] weighting of soil types
    !LOCAL
  ...
  subroutine ovr_src_snk_mss_AlG01_get( &
    ...
    use dstgrd, only: dst_src_nbr, dst_nbr, dst_nbr      !dst dimensions and sizes
    use dstpsd, only:ovr_src_snk_frc  !Fractional overlap source and sink
    
    implicit none
    
    ! OUTPUT
    real(r8), intent(out):: ovr_src_snk_mss(dst_src_nbr,dst_nbr) ! O [frc] mass overlap source sink
    real(r8), intent(out):: ovr_src_snk_mss_ttl                 ! O [frc] total overlap source and sink
    real(r8), intent(out):: mss_frc_trn_dst_src(dst_nbr)         ! O [frc] fracion of transported dust in sink bins
    ! INPUT
    real(r8), intent(in) :: mss_frc_src(dst_src_nbr)             ! I [frc] mass fraction of each source mode
    !LOCAL
    ...
    do snk_idx=1,dst_nbr
       do src_idx=1,dst_src_nbr
          ...
          mss_frc_trn_dst_src(snk_idx) = & ! [frc] Fraction of transported dust mass at source
               mss_frc_trn_dst_src(snk_idx)+ovr_src_snk_mss(src_idx,snk_idx)
    ...
    mss_frc_trn_dst_src(:)= & ! [frc] Fraction of transported dust mass at source
         mss_frc_trn_dst_src(:) &
         /max(1.e-20,ovr_src_snk_mss_ttl)
  ...
  subroutine flx_mss_vrt_dst_ttl_AlG01_get( &
    ...
    use dstgrd, only: wnd_frc_nbr, bln_nbr 

    implicit none
    
    ! OUTPUT
    real(r8), intent(out) :: dst_slt_flx_rat_ttl ! O [m-1] Total horizontal soil flux
    real(r8), intent(out) :: flx_mss_vrt_dst_ttl ! O [kg m-2 s-1] total vertical dust mass flux
    ! INPUT
    real(r8), intent(in)  :: flx_mss_hrz_slt_ttl                         ! I [kg m-1 s-1] total horizontal mass flux of soil
    real(r8), intent(in)  :: dst_slt_flx_rat_ttl_lut(wnd_frc_nbr,bln_nbr) ! I [m-1] ratio of vertical to horizontal flux
    real(r8), intent(in)  :: wnd_frc                   ! I [m/s] wind friction speed
    real(r8), intent(in)  :: sfc_frc_bln(bln_nbr) ! [frc]
    !LOCAL
    ...
    ! INITIALIZE
    dst_slt_flx_rat_ttl = 0._r8
    
    do soil_idx=1,bln_nbr
       wnd_frc_idx = min(100,int(100._r8*wnd_frc+0.5_r8)) !wind friction speed to look up
          
       dst_slt_flx_rat_ttl= &                         !ratio equals
            dst_slt_flx_rat_ttl &                     !old ratio 
            +sfc_frc_bln(soil_idx) &                   !plus contribution from each soiltype
            *dst_slt_flx_rat_ttl_lut(wnd_frc_idx,soil_idx)     !And their respective ratio at that wind speed

    enddo

    flx_mss_vrt_dst_ttl = &           !Vertical dust flux equals
         flx_mss_hrz_slt_ttl &       !horizontal soil flux times
         *dst_slt_flx_rat_ttl        !Ratio just looked up above
  ...
  subroutine flx_mss_hrz_slt_ttl_AlG01_get(   &
    ...
    !// ------------------------------------------------------------------
    !use pmgrid, only:plon,plond
    use dstgrd, only: wnd_frc_nbr, bln_nbr       !We need dust grid sizes; Here in particular: bln_nbr
    implicit none
    
    !Input
    real(r8), intent(in)            :: flx_mss_hrz_slt_ttl_lut(wnd_frc_nbr,bln_nbr) ![kg m-1 s-1]Look up table for horizontal flux
    real(r8), intent(in)            :: sfc_frc_bln(bln_nbr)                   ![frc] Surface fraction of soil blends
    real(r8), intent(in)            :: wnd_frc                               ![m s-1] Wind friction speed

    !Output
    real(r8), intent(out)           :: flx_mss_hrz_slt_ttl                   ![kg m-1 s-1] Horizontal saltation flux
    ...
    do bln_idx=1,bln_nbr
       ...
       wnd_frc_idx = min(100,nint(wnd_frc*100.0_r8))  

       !Add up horizontal saltation flux
       flx_mss_hrz_slt_ttl = flx_mss_hrz_slt_ttl      & !Horizontal flux equals
            +flx_mss_hrz_slt_ttl_lut(wnd_frc_idx,bln_idx) & !The looked up horizontal flux for a soil type
            *sfc_frc_bln(bln_idx)                        !weighted by the fraction of that soil type at this longitude
  ...
  subroutine flx_mss_vrt_dst_prt( &
    ...
    !// Amund Sovde, October 2009
    !// ------------------------------------------------------------------
    use dstgrd, only: dst_src_nbr,dst_nbr ! [mdl] Dust grid sizes
    !++alfgr use dstaer,only:ovr_src_snk_mss ! [mdl] Aerosol microphysical properties
    !use pmgrid ! [mdl] Spatial resolution parameters
    implicit none
    ! Input
    logical,intent(in)::flg_mbl    ! I [flg] Mobilization candidate flag
    real(r8),intent(in)::flx_mss_vrt_dst_ttl ! I [kg m-2 s-1] Total vertical mass flux of dust
    real(r8),intent(in)::ovr_src_snk_mss(dst_src_nbr,dst_nbr) ![frc] Overlap fractions 
    ! Output
    real(r8),intent(out)::flx_mss_vrt_dst(dst_nbr) ! O [kg m-2 s-1] Vertical mass flux of dust
    integer rcd               ! [rcd] Return success code
    ! Local
    ...
    if (flg_mbl) then
       do snk_idx=1,dst_nbr
          do src_idx=1,dst_src_nbr
             flx_mss_vrt_dst(snk_idx)= & ! [kg m-2 s-1] 
                  flx_mss_vrt_dst(snk_idx)+ &
  ...
  subroutine lnd_frc_mbl_get( &
    ...
    !// Rewritten for Oslo CTM3, works on only one (lon_idx,lat_idx).
    !// Removed longitude loop.
    !// Added lat_idx,lon_idx as input.
    !//
    !// Amund Sovde, October 2009
    !// ------------------------------------------------------------------
    ...
    real(r8),intent(in)::lnd_frc_dry ! I [frc] Dry land fraction
    real(r8),intent(in)::oro ! I [frc] Orography
    real(r8),intent(in)::snw_frc ! I [frc] Fraction of surface covered by snow
    real(r8),intent(in)::tpt_soi ! [K] Soil temperature
    real(r8),intent(in)::tpt_soi_frz ! [K] Temperature of frozen soil
    real(r8),intent(in)::vai_dst ! I [m2 m-2] Vegetation area index, one-sided
    ! Output
    integer,intent(out)::mbl_nbr ! O [nbr] Number of mobilization candidates
    logical,intent(out)::flg_mbl ! O [flg] Mobilization candidates
    real(r8),intent(out)::lnd_frc_mbl ! O [frc] Bare ground fraction
    ! Input/Output
    ! Local
    ...
    flg_mbl = .false. ! [flg] Mobilization candidates
    ...
    if (sfc_typ_idx <= 1.or. & ! Inland lakes and land ice
         ...
         tpt_soi < tpt_soi_frz) then ! Frozen soil
       lnd_frc_mbl=0.0_r8 ! [frc] Bare ground fraction
       ...
       if (flg_vai_tvbds) then
          ...
          lnd_frc_mbl = & ! O [frc] Bare ground fraction
               1.0_r8-min(1.0_r8,min(vai_dst,vai_mbl_thr)/vai_mbl_thr)
          ...
          do sgs_idx=1,3
             ...
             ! endif            ! endif dbg
             lnd_frc_mbl = lnd_frc_mbl+pln_frc_sgs*pln_frc_mbl ! [frc]
          end do            ! end loop over number of possible plant types
       endif               ! not flg_vai_tvbds
    ...
    lnd_frc_mbl=   &
         ...
         *lnd_frc_dry & ! Account for wetlands, lake, ocean, ice
         *(1.0_r8-snw_frc) ! Account for snow coverage
       
    if ((lnd_frc_mbl > 1.0_r8).or.(lnd_frc_mbl < 0.0_r8)) then
       write(6,'(a,i3,a,i3,a,f15.8)')  &
            'dst: lnd_frc_mbl_get(): lat_idx:',lat_idx,' lon_idx:',lon_idx,'lnd_frc_mbl:',lnd_frc_mbl
       stop
    endif                  ! endif out of bounds error
       
    if (lnd_frc_mbl > 0.0_r8) then
       flg_mbl = .true. ! [flg] Mobilization candidates
