...
contains
  ...
  subroutine get_new_events(JYEAR,JMON,JDATE,NDAY,NDAYI, NMET,NOPS)
    ...
    if (NDAY.eq.NDAYI .and. NMET.eq.1 .and. NOPS.eq.1) then
       write(*,'(a)') '* oc_troccinox_geo.f90: Initializing troccinox_geo measurements'
       ...
       !// The flight list
       filename='Indata_CTM3/observations/troccinox2_geophysica.dat'
       ...
       if (file_err .ne. 0) then
          ...
          write(*,'(a)')   '    Will not simulate troccinox geophysica flights!'
          return
       else
    ...
    write(*,'(a)') '* oc_troccinox_geo.f90: Looking up new measurements'
    ...
    do while (temp_itau < now_itau)
       ...
       if (file_err .ne. 0) then
          ...
          if (.not.endoffile) print*,'End of troccinox_geo input file'
          endoffile=.true.
          nodata = .true.
          exit
       end if
          ...
          do while (okdata)
             ...
             if (temp_itau .ge. now_itau .and. temp_itau .lt. now_itau_p) then
                ...
                if (n_event .gt. maxOPD) then
                   print*,'oc_troccinox_geo.f90: n_event > maxOPD'
                   stop
                end if
                ...
                if (iii.lt.0 .or. jjj.lt.0) then
                   print*,'* oc_troccinox_geo: ERROR: III/JJJ is wrong:',iii,jjj
                   stop
                end if
                ...
                if (nbxfrac .gt. 1._r8) then
                   print*,'oc_troccinox_geo.f90: Something wrong with lon:'
                   print*,nbxfrac,temp_lon,XDGRD(iii)
                   print*,iii,nii
                    stop
                end if
                if (nbyfrac .gt. 1._r8) then
                   print*,'oc_troccinox_geo.f90: Something wrong with lat:'
                ...
                if (temp_itau < now_itau) then
                   print*, '* oc_troccinox_geo.f90: temp_itau < now_itau'
    ...
    if (file_err .ne. 0) then
       ...
       print*,temp_itau, now_itau
       print*,'End of oc_troccinox_gep.f90 input file?'
       return
    end if

    write(*,'(a)') '-------------------------------------------------' &
         //'----------------------'
    write(*,'(a,i5,a)')'TROCCINOX GEOPHYSICA: ',n_event,' events from file.'
    write(*,'(a)') '-------------------------------------------------' &
         //'----------------------'
      ...
      do tops = 1, 2
        ...
        if ( (tops.eq.1 .and. (.not. LLAST)) .or. &
          ...
          if (tops .eq. 1) then
            ...
            print*,'* oc_troccinox_geo.f90: tops wrong',tops
            stop
          end if
          ...
          if (temp_itau .ge. min_itau .and. &
            ...
            if (verbose) print*,'TROCCINOX_GEO',nev,tops,lfirst,llast,min_itau,&
                 temp_itau,max_itau
                ...
                if (trsp_idx(114) .gt. 0) then
                  ...
                  Do L = LMTROP(I,J)+1, LPAR
                    ...
                    if (trsp_idx(46).gt.0) then
                      ...
                      print*, '* oc_troccinox_geo.f90: CH4 not available'
                      stop
                    end if
                    !// Get H2
                    if (trsp_idx(113).gt.0) then
                      ...
                      print*, '* oc_troccinox_geo.f90: H2 not available'
                      stop
                    end if
  ...
  subroutine flight_data_to_file(NDAY)
    ...
    filename = trim(RESULTDIR)//'troccinox_geo_day'//cday//'.dta'
    ...
    do n = 1, n_event
       if (verbose) print*,'TROCCINOX GEOPHYSICA to file; writing event',N
  ...
  !// --------------------------------------------------------------------
  subroutine troccigeo_master(JYEAR, JMON, JDATE,NDAY,NMET,NOPS, NDAYI)
    !// --------------------------------------------------------------------
    ...
    !// --------------------------------------------------------------------
  end subroutine troccigeo_master
  !// --------------------------------------------------------------------

  !// --------------------------------------------------------------------
end module troccinox_geo
!//=========================================================================
