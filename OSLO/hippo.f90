...
!//=========================================================================
!// HIPPO output from Oslo CTM3
!//=========================================================================
module hippo
  !// ----------------------------------------------------------------------
  !// MODULE: hippo
  !// DESCRIPTION: Routines for putting out HIPPO data.
  !//
  !// Ole Amund Sovde, March 2010
  !// ----------------------------------------------------------------------
  use cmn_precision, only: r8, r4
  !// ----------------------------------------------------------------------
  !// Routine for putting out 10-sec HIPPO data (measured 2009-2011).
  !//
  ...
  !// ----------------------------------------------------------------------
  character(len=*), parameter, private :: f90file = 'hippo.f90'
  !// ----------------------------------------------------------------------
  save
  private
  public hippo_master
...
contains
  ...
  subroutine get_new_events(JYEAR,JMON,JDATE,NDAY,NDAYI, NMET,NOPS,LNEWM)
    ...
    if (LNEWM) then
       ...
       !// The flight list
       filename='Indata_CTM3/observations/hippo_all_10s_mbcval.txt'
       ...
       if (file_err .ne. 0) then
          write(6,'(a,a)') f90file//':'//subr// &
               ...
               ', will not simulate hippo flights!'
          return
       else
    ...
    write(6,'(a)') f90file//':'//subr//': Initializing hippo measurements'
    ...
    write(6,'(a)') '-------------------------------------------------' &
         //'----------------------'
    write(6,'(a,i5,a)')'HIPPO: ',n_event,' events from file.'
    write(6,'(a)') '-------------------------------------------------' &
         //'----------------------'
  ...
  !// ----------------------------------------------------------------------
  subroutine hippo_output(JYEAR,JMON,JDATE, NDAY, NMET, NOPS)
    !// --------------------------------------------------------------------
    ...
    !// --------------------------------------------------------------------
    character(len=*), parameter :: subr = 'hippo_output'
    !// ----------------------------------------------------------------------
      ...
      do tops = 1, 2
        ...
        if ( (tops.eq.1 .and. (.not. LLAST)) .or. &
          ...
          if (temp_itau .ge. min_itau .and. &
            ...
            if (verbose) print*,'HIPPO',nev,tops,lfirst,llast,min_itau,&
                 temp_itau,max_itau
    ...
    !// --------------------------------------------------------------------
  end subroutine hippo_output
  !// ----------------------------------------------------------------------



  !// ----------------------------------------------------------------------
  subroutine hippo_data_to_file(NDAY)
    !// --------------------------------------------------------------------
    ...
    filename = trim(RESULTDIR)//'hippo_day'//cday//'.dta'
    ...
    do n = 1, n_event
       if (verbose) print*,'HIPPO to file; writing event',N
    ...
    !// --------------------------------------------------------------------
  end subroutine hippo_data_to_file
  !// ----------------------------------------------------------------------



  !// ----------------------------------------------------------------------
  subroutine hippo_master(JYEAR, JMON, JDATE,NDAY,NMET,NOPS, NDAYI,LNEWM)
    !// --------------------------------------------------------------------
    ...
    if (NDAY.ne.NDAYI .and. NMET.eq.1 .and. NOPS.eq.1) then
       ...
       call hippo_output(curyear,curmonth,curdate,NDAY-1, NRMETD+1, 1)

       !// Write to file (NDAY-1 since we save previous day)
       call hippo_data_to_file(NDAY-1)
    ...
    !// Process profiles
    call hippo_output(JYEAR,JMON,JDATE,NDAY, NMET, NOPS)


    !// --------------------------------------------------------------------
  end subroutine hippo_master
  !// ----------------------------------------------------------------------
