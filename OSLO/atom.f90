...
!//=========================================================================
!// ATOM output from Oslo CTM3
!//=========================================================================
module atom
  !// ----------------------------------------------------------------------
  !// MODULE: atom
  !// DESCRIPTION: Routines for putting out ATOM data.
  !//
  !// Ole Amund Sovde, March 2010
  !// ----------------------------------------------------------------------
  use cmn_precision, only: r8, r4
  !// ----------------------------------------------------------------------
  !// Routine for putting out 10-sec ATOM data (measured 2009-2011).
  !//
  ...
  !integer,parameter :: ntracer=47
  !integer,dimension(ntracer),parameter :: out_comps = &
  !     (/6,62,63,64,65,73,165,166,167,168,169,170,171,172,173, &
  !     174,175,176,177,178,179,180,181,182,183,184,185,186,187,188,&
  !     189,190,191,230,231,232,233,234,235,236,237,240,241,242,243,244,245/)
  integer,parameter :: ntracer=63
  integer,dimension(ntracer),parameter :: out_comps = &
       (/  1,  4,  6,  7,  8,  9, 13, 15, 17, 20, &
          21, 41, 42, 43, 44, 46, 48, 62, 63, 64, &
          65, 73,165,166,167,168,169,170,171,172, &
         173,174,175,176,177,178,179,180,181,182, &
         183,184,185,186,187,188,189,190,191,230, &
  ...
  !// ----------------------------------------------------------------------
  character(len=*), parameter, private :: f90file = 'atom.f90'
  !// ----------------------------------------------------------------------
  save
  private
  public atom_master
...
contains
  ...
  subroutine get_new_events(JYEAR,JMON,JDATE,NDAY,NDAYI, NMET,NOPS,LNEWM)
    ...
    if (LNEWM) then
       ...
       !// The flight list
       filename='Indata_CTM3/observations/atom_mbcval_allcomp.txt'
       ...
       if (file_err .ne. 0) then
          write(6,'(a,a)') f90file//':'//subr// &
               ': Could not open file: '//trim(filename)// &
               ', will not simulate atom flights!'
          return
       else
          write(6,'(a)') f90file//':'//subr//': Reading file: '//trim(filename)
       end if
    ...
    write(6,'(a)') f90file//':'//subr//': Initializing atom measurements'
    ...
    do while (temp_itau < now_itau)
       ...
       !// If end of file...
       if (file_err .ne. 0) then
          write(6,'(a,i5,2i16)') f90file//':'//subr// &
               ': Read error1 - end of atom input file: ', &
               file_err,temp_itau, now_itau
          endoffile=.true.
          nodata = .true.
          close(file_id)
          exit
    ...
    do NM = 1,NRMETD
       do NO = 1,NROPSM
          ...
          do while (okdata)
             ...
             if (file_err .ne. 0) then
                write(6,'(a,i5,2i16)') f90file//':'//subr// &
                     ': Read error2 - reached end of atom input file?: ', &
                     file_err,temp_itau, now_itau
                endoffile = .true.
                close(file_id)
                exit
             ...
             if (temp_itau .ge. now_itau .and. temp_itau .lt. now_itau_p) then
                ...
                if (n_event .gt. maxOPD) then
                   write(6,'(a,2i8)') f90file//':'//subr//': n_event > maxOPD',&
                        n_event, maxOPD
                   stop
                end if

                write(6,'(a,2(i4,2i2.2,1x,3i2.2,1x),2i16,i6)')&
                     f90file//':'//subr//': OK ', &
                ...
                if (nbxfrac .gt. 1._r8) then
                   write(6,'(a,5es16.6,2i5)') f90file//':'//subr// &
                        ': Something wrong with lon: ',&
                        nbxfrac,temp_lon,xdgrd(iii),ctmlon(iii),deltalon,iii,nii
                   stop
                end if
                if (nbyfrac .gt. 1._r8) then
                   write(6,'(a,3es16.6,2i5)') f90file//':'//subr// &
                        ': Something wrong with lat: ', &
                        nbyfrac,temp_lat,YDGRD(jjj),jjj,njj
                   stop
                end if
                ...
                if (temp_itau < now_itau) then
                   write(6,'(a)') f90file//':'//subr//': temp_itau < now_itau'
                   write(6,'(a)') '  Should have been read off at start'
                   write(6,'(a)') '  Something is wrong - stopping'
                   stop
                end if
                if (verbose) write(6,'(a,2(i4,2i2.2,1x,3i2.2,1x),2i16)') &
                     f90file//':'//subr//': NOT ', &
                     now_date,temp_idate, now_itau,temp_itau
    ...
    if (file_err .ne. 0) then
       write(6,'(a,i5,2i16)') f90file//':'//subr// &
            ': Read error3 - reached end of atom input file?: ', &
            file_err,temp_itau, now_itau
       return
    end if
    ...
    write(6,'(a)') '-------------------------------------------------' &
         //'----------------------'
    write(6,'(a,i5,a)') f90file//':'//subr//': ATOM: ', &
         n_event,' events from file.'
    write(6,'(a)') '-------------------------------------------------' &
         //'----------------------'
  ...
  !// ----------------------------------------------------------------------
  subroutine atom_output(JYEAR,JMON,JDATE, NDAY, NMET, NOPS)
    !// --------------------------------------------------------------------
    ...
    !// --------------------------------------------------------------------
    character(len=*), parameter :: subr = 'atom_output'
    !// --------------------------------------------------------------------
      ...
      do tops = 1, 2
        ...
        if ( (tops.eq.1 .and. (.not. LLAST)) .or. &
          ...
          if (temp_itau .ge. min_itau .and. &
            ...
            if (verbose) print*,'ATOM',nev,tops,lfirst,llast,min_itau,&
                 temp_itau,max_itau
  ...
  !// ----------------------------------------------------------------------
  subroutine atom_data_to_file(NDAY)
    !// --------------------------------------------------------------------
    ...
    integer :: ifnr, n
    !// --------------------------------------------------------------------
    character(len=*), parameter :: subr = 'atom_data_to_file'
    !// --------------------------------------------------------------------
    ...
    !// File name
    IF(NDAY.ge.366) THEN
       write(cday,'(i3.3)') NDAY-365
    ELSE
       write(cday,'(i3.3)') NDAY
    ENDIF
    !write(cday(1:3),'(i3.3)') NDAY
    filename = trim(RESULTDIR)//'atom_day'//cday//'.dta'
    ...
    do n = 1, n_event
       if (verbose) write(6,'(a,i8)') '  ATOM to file; writing event',N
    ...
    !// --------------------------------------------------------------------
  end subroutine atom_data_to_file
  !// ----------------------------------------------------------------------



  !// ----------------------------------------------------------------------
  subroutine atom_master(JYEAR, JMON, JDATE,NDAY,NMET,NOPS, NDAYI,LNEWM)
    !// --------------------------------------------------------------------
    ...
    if (NDAY.ne.NDAYI .and. NMET.eq.1 .and. NOPS.eq.1) then
       ...
       call atom_output(curyear,curmonth,curdate,NDAY-1, NRMETD+1, 1)

       !// Write to file (NDAY-1 since we save previous day)
       call atom_data_to_file(NDAY-1)
    ...
    !// Process profiles
    call atom_output(JYEAR,JMON,JDATE,NDAY, NMET, NOPS)


    !// --------------------------------------------------------------------
  end subroutine atom_master
  !// ----------------------------------------------------------------------
