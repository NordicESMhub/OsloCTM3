...
c     character*10, parameter ::  MODEL = 'EC_T42_L40'
...
c--T319_L60 run at full resolution and L57 or L60, Gaussian grid N160
      character*10, parameter ::  MODEL = 'ECT319_L60'

c---resolution of input met fields
c     integer, parameter :: IPARW=128, JPARW=64,  LPARW=40   !T42L40 EC grid
...
c     integer, parameter :: IPARW=128, JPARW=64,  LPARW=60   !T42L60 EC grid
      integer, parameter :: IPARW=640, JPARW=320, LPARW=60   !319L60 EC grid
...
c     integer, parameter :: NSPGT = 3-(IPARW/300)     ! T42
c     integer, parameter :: NTRUNW = (IPARW-1)/NSPGT  ! T42
      integer, parameter :: NTRUNW = 319              ! 1x1  and T319
      integer, parameter :: NRNM = (NTRUNW+1)*(NTRUNW+2)
      integer, parameter :: NMMAX = (NTRUNW+1)*(NTRUNW+4)/2

C  FFT transforms
c     integer, parameter :: NRFFT = 32            ! T42
c     integer, parameter :: FFTPTS = IPARW+10     ! T42
c     integer, parameter :: NRFFT = 4             ! 1X1
c     integer, parameter :: FFTPTS = 3*(NTRUNW+2) ! 1X1
      integer, parameter :: NRFFT = 32            ! T319
      integer, parameter :: FFTPTS = 3*(NTRUNW+2) ! T319
...
c     integer, parameter :: IPAR=128, JPAR=64, LPAR=37   !**  ECT42L40 grid
...
c     integer, parameter :: IPAR=180, JPAR=90,  LPAR=37  !**  EC2x2 grid
      integer, parameter :: IPAR=640, JPAR=320, LPAR=57  !**  ECT319L60 grid
...
c---max number of tracers (CHEM sets NTM = no. of used tracers):
      integer, parameter ::  NPAR=4   ! if ASAD is on, NPAR=JITR+JITE+JIIN

c---Dimension for 2-D emission tables
      integer, parameter ::  NE2DS=1     ! 1: mass only, 6: mass plus moments
c     integer, parameter ::  NE2DS=6     ! 1: mass only, 6: mass plus moments
...
c  Master Chemistry Switch
      logical, parameter ::  LPASAD=.false.    ! ASAD switch
c
      integer, parameter ::
     &  JITR = 4,  ! Number of TR tracers: implicit chem. and transport
     &  JITE = 0,  ! Number of TR tracers: explicit chem. and transport
     &  JISS = 0,  ! Number of SS species: steady state, no transport
     &  JITS = 0,  ! Number of TS species: implicit chem., no transport
     &  JICT = 0,  ! Number of CT/CF species:  constant, no transport
     &  JIIN = 0,  ! Number of IN tracers: no chemistry, transport
     ...
     &  JITS1  = max(JITS,1)               ! in case jits=0
      ...
      integer, parameter ::
     &  JPBK = 0,       ! Number of bimolecular reactions. May be 0.
     &  JPTK = 0,       ! Number of trimolecular reactions.
     &  JPPJ = 0,       ! Number of photolysis reactions.
